...
class Common {
  ...
  createMigrationTable() {
    return new Promise((resolve, reject) => {
      this.db.client.execute(migration_settings.createMigrationTable, null, { prepare: true }, function (err, response) {
        if (err) {
          reject(err);
        }
        resolve(response);
  ...
  getMigrations() {
    return new Promise((resolve, reject) => {
      this.filesRan = {};
      let self = this;
      this.db.client.execute(migration_settings.getMigration, null, { prepare: true }, function (err, alreadyRanFiles) {
        if (err) {
          reject(err);
        } else {
          let filesRan = {};
